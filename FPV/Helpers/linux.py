...
class Linux(BaseService):
    ...
    def path_part_contains_invalid_characters(part):
        invalid_pattern = re.compile(f"[{re.escape(Linux.invalid_characters)}]")
    ...
    def check_if_valid(self):
        ...
        for idx, part in enumerate(parts):
            ...
            invalid_character = Linux.path_part_contains_invalid_characters(part)
            if invalid_character:
                raise ValueError(
                    f'Invalid character "{invalid_character.group()}" found in "{part}". '
                    f'Please avoid using: {Linux.invalid_characters}'
                )
